...
def read_in_buddies(target_station: utils.Station, station_list: pd.DataFrame, 
                    initial_neighbours: np.array, variable: utils.Meteorological_Variable,
                    diagnostics: bool = False, plots: bool = False) -> tuple[int, np.array]:
    """
    ...
    :returns: (int, array) number of buddies, and the buddy data
    ...
    return buddy_count, all_buddy_data


#************************************************************************
def calculate_data_spread(target_station: utils.Station,
                          differences: np.array) -> np.array:
    """
    Find spread of differences on monthly basis (with minimum value)  

    :param Station target_station: station to run on    
    :param np.array differences: array of differences of buddy data to target data
    
    :returns: np.array of the spread of the differences
    """
...
#************************************************************************
def adjust_pressure_for_tropical_storms(dubious: np.ma.array, initial_neighbours: np.array,
                                        differences: np.array, spreads: np.array) -> np.array:
    """
    ...
    :param np.ma.array dubious: locations where differences are greater than expected
    :param array initial_neighbours: input neighbours (ID, distance) pairs
    ...
    :returns: np.array of locations where target values are dubious given the neighbours  
    """
...
def neighbour_outlier(target_station: utils.Station, initial_neighbours: np.array,
    ...
    buddy_count, all_buddy_data = read_in_buddies(target_station, station_list, initial_neighbours,
