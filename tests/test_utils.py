...
def test_read_cloud_oktas() -> None:
    """Test that cloud okta reading works"""

    df = pd.DataFrame({
        "sky_cover_layer_2": ["FEW:01", "SCT:03", "BKN:05",
                              "OVC:08", np.nan, "FEW:02"]
    })

    result = utils.read_cloud_oktas(df, "sky_cover_layer_2")

    # Name of expected series is just 1, as numbers autogenerated by str.split
    expected = pd.Series(["01", "03", "05",
                          "08", np.nan, "02" ], name=1)
